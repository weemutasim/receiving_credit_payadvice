...
class Recevingvoucher {
  ...
  Future<Uint8List> genPDFRecevingvoucher({required DateTime date/* , required PdfPageFormat formatter */, required CashAdvance data}) async {
    cashAdvance = data;
      ...
      return pw.Row(
        children: List.generate(listDate.length, (index) {
          ...
          return pw.Container(
            ...
            child: chk == 0 ? pw.Text(data, style: pw.TextStyle(font: traJanPro, fontSize: 4)) : pw.Text(dateNo, style: pw.TextStyle(font: traJanPro, fontSize: 4.5))
          );
        }) 
      );
    }
              ...
              return pw.Container(
                ...
                child: pw.Column(
                  ...
                  children: [
                    ...
                    pw.Text(index == 1 ? '${cashAdvance!.name?.split(' ')[0]}' : '________________________', style: pw.TextStyle(font: pgVim, fontSize: 5)),
                    pw.SizedBox(height: 4),
                    pw.Text(index == 1 ? '(     ${cashAdvance!.name}     )' : '(                                        )', style: pw.TextStyle(font: pgVim, fontSize: 5)),
                    pw.SizedBox(height: 4),
                    pw.Text(index == 1 ? 'วันที่ /Date   ${dateformat(date: date, type: 'dsn')}' : 'วันที่ /Date _______________', style: pw.TextStyle(font: pgVim, fontSize: 5)),
                  ]
                )
              );
            })
